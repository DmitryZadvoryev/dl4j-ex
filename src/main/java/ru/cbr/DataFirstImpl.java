...
public class DataFirstImpl {
    ...
    public void addHasAttrValue(String key, boolean value){
        hasAttributes.put(key, value);
    }

    public String toCsv(){
        String tags = this.tags.values().stream().map(b -> Boolean.toString(b)).collect(Collectors.joining(","));
        String attrValues = this.attributeValues.values().stream().collect(Collectors.joining(","));
        String hasAttr = this.hasAttributes.values().stream().map(b -> Boolean.toString(b)).collect(Collectors.joining(","));
